...
/**
 * For a variable in a tree : computes the percentage of different predictions between the OBB and permuted OOB
 */
public class DifferenceInPrediction implements PermutationIndexIF<Double> {
	
	int nbDiffPred, count;
	
	public DifferenceInPrediction(){
		reInit();
	}
	...
	public void addPoint(int pred, int permPred, int label, boolean isVariableInTree, int pointIndex) {
		// if !isVarInTree, pred == permPred --> nothing changes
		if(pred != permPred) nbDiffPred++;
		count++;
	}
	...
	public Double getIndex() {
		...
		return ((double) nbDiffPred) / count;
	}
}
